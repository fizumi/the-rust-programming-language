mod list3; mod smart_p; mod ref_cell;
...
mod tests_bk {
    ...
    impl Messenger for MockMessenger {
        // ライブラリの Messenger の send の型は `&self` だけど、テストのために `&mut self` にしたい
        fn send(&self, message: &str) {
            // self.sent_messages.push(String::from(message)); // ❌ push は self を変更する
        }
    }
